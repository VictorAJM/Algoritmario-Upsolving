/**
 * Description: Used infrequently. Constructs
 *  minimal deterministic 
 * finite automaton (DFA) that recognizes 
 * all suffixes of a string.
 * \texttt{len} corresponds to the maximum 
 * length of a string in
 * the equivalence class, \texttt{pos} 
 * corresponds to
 * the first ending position of such a 
 * string, \texttt{lnk}
 * corresponds to the longest suffix that 
 * is in a different class.
 * Suffix links correspond to suffix tree 
 * of the reversed string!
 */

struct SuffixAutomaton {
  int N = 1; vi lnk{-1}, len{0}, pos{-1}; 
  // suffix link, max length of state,
  // last pos of first occurrence of state
  V<map<char,int>> nex{1}; 
  V<bool> isClone{0};
  ...
  void init(str s) { 
    int p = 0; each(x,s) p = add(p,x); 
  } /// add string to automaton
  // inverse links
  void genIlnk() {
    iLnk.rsz(N);FOR(v,1,N)iLnk[lnk[v]].pb(v);
    }
  // APPLICATIONS
  void getAllOccur(vi& oc, int v) {
    // terminal position
    if (!isClone[v]) oc.pb(pos[v]); 
    each(u,iLnk[v]) getAllOccur(oc,u); 
  }
  // get all occurrences of s in automaton
  vi allOccur(str s) { 
    int cur = 0;
    each(x,s) {
  ...
  ll getDistinct(int x) {
    ...
    distinct[x]=1;
    each(y,nex[x]) 
      distinct[x]+=getDistinct(y.s);
    return distinct[x]; 
  }
  // # distinct substrings including empty
  ll numDistinct() { 
    distinct.rsz(N); return getDistinct(0); 
  }
  // assert(numDistinct()==numDistinct2());
  ll numDistinct2() { 
    ll ans = 1; 
    FOR(i,1,N) ans += len[i]-len[lnk[i]];
    return ans; } 
};
